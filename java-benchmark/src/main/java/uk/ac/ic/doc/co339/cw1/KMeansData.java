...
public class KMeansData {
  ...
  public static Collection<MyBenchmark.Centroid> getRandomCentroidDataSet(int numCentroids) {
    List<MyBenchmark.Centroid> centroidList = new LinkedList<>();
    for (int i = 1 ; i <= numCentroids ; i++) {
      Random rand = new Random();
      centroidList.add(
          new MyBenchmark.Centroid( i,  (-60 + rand.nextDouble() * 120), (-60 + rand.nextDouble() * 120)));
    }
    return centroidList;
  }

  public static Collection<MyBenchmark.Centroid> getDefaultCentroidDataSet() {
    List<MyBenchmark.Centroid> centroidList = new LinkedList<MyBenchmark.Centroid>();
    for (Object[] centroid : CENTROIDS) {
      centroidList.add(
          new MyBenchmark.Centroid((Integer) centroid[0], (Double) centroid[1], (Double) centroid[2]));
    }
    return centroidList;
  }

  public static Collection<MyBenchmark.Point> getDefaultPointDataSet() {
    List<MyBenchmark.Point> pointList = new LinkedList<MyBenchmark.Point>();
    for (Object[] point : POINTS) {
      pointList.add(new MyBenchmark.Point((Double) point[0], (Double) point[1]));
    }
    return pointList;
  }
